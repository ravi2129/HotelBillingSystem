...
def click_send():
    ...
    if offer =='':
        ...
        alldb = mysql.connector.connect(host='localhost', user='root', password='Admin1234', database='hotel')
        allcur = alldb.cursor()
        ...
        for dest in emails:
            ...
            s.login("herepython506@gmail.com","python506project")
            message = "Subject : {}\n\n{}".format(subject, offer)
            s.sendmail("email@gmail.com", dest, message)
            s.quit()


title_label = Label(root, text="---------  OFFERS EMAIL  ---------", fg='white',width=80, height=2, font=('Orbitron', 17), bg="black")

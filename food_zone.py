...
root.title("SKY HIGH FOOD-ZONE")
root.geometry("1000x1000")

root.configure(background="#4DA8DA")

Tops = Frame(root, bg='dark blue', bd=25, pady=20, relief=GROOVE)
Tops.pack(side=TOP)

lblTitle = Label(Tops, font=('arial', 30, 'bold'), text='SKY HIGH FOOD-ZONE', bd=15, bg='sky blue',
...
def Receipt():
    ...
    # extracted data to string
    receiptRefstr = 'Receipt Ref:\t\t\t'+Receipt_Ref.get() + '\t' + Date_of_Order.get() + '\n'
    unitstr = 'Unit\t\t\t\t'+"Total of Unit \n"
    cocktailstr = 'Cocktail:\t\t\t\t\t' + cocktail.get() + '\n'
    iced_tea_str =  'Iced Tea:\t\t\t\t\t' + iced_tea.get()+'\n'
    hot_chocolate_str = 'Hot Chocolate:\t\t\t\t\t' + hot_chocolate.get()+'\n'
    orange_juice_str = 'Orange Juice:\t\t\t\t\t' + orange_juice.get()+'\n'
    milkshake_str = 'Milk Shake:\t\t\t\t\t' + milkshake.get()+'\n'
    mountain_dew_str = 'Mountain Dew:\t\t\t\t\t' + mountain_dew.get()+'\n'
    sting_str = 'Sting:\t\t\t\t\t' + sting.get()+'\n'
    cobra_str = 'Cobra:\t\t\t\t\t' + cobra.get()+'\n'
    fried_chicken_str = 'Fried Chicken:\t\t\t\t\t' + fried_chicken.get()+'\n'
    kare_kare_str = 'Kare Kare:\t\t\t\t\t' + kare_kare.get()+'\n'
    crispy_pata_str = 'Crispy Pata:\t\t\t\t\t' + crispy_pata.get()+'\n'
    sinigang_baboy_str = 'Sinigang baboy:\t\t\t\t\t' + sinigang_baboy.get()+'\n'
    sinigang_hipon_str = 'Sinigang Hipon:\t\t\t\t\t' + sinigang_hipon.get()+'\n'
    bicol_express_str = 'Bicol Express:\t\t\t\t\t' + bicol_express.get()+'\n'
    asparagus_tofu_str = 'Asparagus Tofu:\t\t\t\t\t' + asparagus_tofu.get()+'\n'
    chicken_sisig_str = 'Chicken Sisig:\t\t\t\t\t' + chicken_sisig.get()+'\n'
    total_of_Drinks_str = 'Total of Drinks:\t\t\t\t' + Total_of_Drinks.get()
    PaidTax_str = '\nTax Paid:\t\t\t\t'+PaidTax.get()+"\n"
    Total_of_Food_str = 'Total of Foods:\t\t\t\t' + Total_of_Food.get()
    SubTotal_str = '\nSubTotal:\t\t\t\t'+str(SubTotal.get())+"\n"
    ServiceCharge_str = 'Service Charge:\t\t\t\t' + ServiceCharge.get()
    TotalCost_str = '\nTotal Cost:\t\t\t\t'+str(TotalCost.get())+"\n"
    # insert data
    doc_ref = db.collection('Receipt').document(randomRef)
    doc_ref.set({
        'Receipt Ref': receiptRefstr,
        'unit': unitstr,
        'cocktail': cocktailstr,
        'iced_tea': iced_tea_str,
        'hot_chocolate': hot_chocolate_str,
        'orange_juice': orange_juice_str,
        'milkshake': milkshake_str,
        'mountain_dew': mountain_dew_str,
        'sting': sting_str,
        'cobra': cobra_str,
        'fried_chicken': fried_chicken_str,
        'kare_kare': kare_kare_str,
        'crispy_pata': crispy_pata_str,
        'sinigang_baboy': sinigang_baboy_str,
        'sinigang_hipon': sinigang_hipon_str,
        'bicol_express': bicol_express_str,
        'asparagus_tofu': asparagus_tofu_str,
        'chicken_sisig': chicken_sisig_str,
        'total_of_Drinks': total_of_Drinks_str,
        'PaidTax': PaidTax_str,
        'Total_of_Food': Total_of_Food_str,
        'SubTotal': SubTotal_str,
        'ServiceCharge': ServiceCharge_str,
        'TotalCost_str': TotalCost_str,
    })
    # set data
